...
---Q.6 Calculate average hotel room rate in different cities ?
...
--- Q.8 Calculate the number of bookings cancelled by customers  in different cities ?
...
--- Q.12 Calculate the total revenue generated in different cities by months name ?
SELECT DATENAME(MONTH, check_in) AS 'Month',City,
COUNT(booking_id) AS Total_Bookings,
SUM(Paid_Amount) AS Total_Revenue_In_City
FROM  [Location] L ,[Hotel] H
WHERE H.hotel_id = L.hotel_id
GROUP BY DATENAME(MONTH, check_in),City
ORDER BY Total_Revenue_In_City DESC;

---Q.13 Calculate the average room rate in different cities ?
SELECT City,ROUND(AVG(Paid_Amount),0) Average_room_rate
FROM  [Location] L ,[Hotel] H
WHERE H.hotel_id = L.hotel_id
GROUP BY City
...
--- Q.14 Find out the total revenue generated in different cities for each month and accordingly arrange the cities in decending order ?
SELECT  MONTH(check_in) AS Month_No,
DATENAME(MONTH, check_in) AS Month_Name,City,
COUNT(booking_id) AS Total_Bookings,
 SUM(Paid_Amount) AS Total_Revenue_In_City
FROM  [Location] L ,[Hotel] H
WHERE H.hotel_id = L.hotel_id
GROUP BY DATENAME(MONTH, check_in),City,MONTH(check_in)
ORDER BY Month_No ASC , Total_Revenue_In_City DESC;
